...
import FetchShare from '../components/FetchShare';
  ...
  const ChatScreen = () => {
    ...
    const fetchMessages = async () => {
      // console.log(":asas")
      // console.log(userId)
      console.log("REEC",recepientId);
      

      try {
        //  for deployment
        // const response = await fetch(
        //   `https://backendmobileapp.onrender.com/messages/${userId}/${recepientId}`
        // );
        const response = await fetch(
          `${api}/messages/${userId}/${recepientId}`
        );
    ...
    useEffect(() => {
      const fetchRecepientData = async () => {
        try {
          //  for deployment
          // const response = await fetch(
          //   `https://backendmobileapp.onrender.com/user/${recepientId}`
          // );
          
          const response = await fetch(
            `${api}/user/${recepientId}`
          );
          const data = await response.json();
    ...
    const handleSend = async (messageType, imageUri) => {
      ...
      try {
        ...
        // const response = await fetch('https://backendmobileapp.onrender.com/messages', {
        ...
        headerLeft: () => (
          ...
          >
            ...
            {selectedMessages.length > 0 ? (
              ...
              <TouchableOpacity onPress={() => navigation.navigate('userfeed',({userId: recepientData._id,recepientName:recepientData.name,recepientImage:`http://10.0.2.2:8000/${recepientData.image}`}))}
                style={{
                  flexDirection: 'row',
                  alignItems: 'center',
                }}
              >
                ...
                <Image
                  ...
                  source={{ uri: `${api}/${recepientData?.image}` }}
                />
                <Text
                  style={{
                    marginLeft: 5,
    ...
    const deleteMessages = async (messageIds) => {
      try {
        // const response = await fetch('https://backendmobileapp.onrender.com/deleteMessages', {
    ...
    //  for handle Image link press
    
    const handleClickLink = ({productId,userId}) => {
      navigation.navigate('like',{ productId, userId});
    ...
    // for getting the productId from the url
        ...
        <ScrollView ref = {scrollViewRef} contentContainerStyle  = {{flexGrow:1}} onContentSizeChange={handleContentSizeChange}>
          {messages.map((item, index) => {
            ...
            if (item.messageType === 'image') {
              {/* const baseUrl = 'https://backendmobileapp.onrender.com'; */}
              {/* const baseUrl = 'https://petservernew.onrender.com' */}
            ...
            if (item.messageType === 'share') {
      const isSelected = selectedMessages.includes(item._id);
        // Extract product ID from linkUrl
  const linkParts = item?.linkUrl.split('/');
  const productId = linkParts[linkParts.length - 1];
      ...
      return (
        ...
        >
        <TouchableOpacity onPress={() => handleClickLink({productId,userId})}>
        <Text
            style={{
              fontSize: 13,
              textAlign: isSelected ? 'right' : 'left',
          ...
          <FetchShare productId= {productId}/>
